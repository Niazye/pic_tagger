...
- **缩略图生成**：使用 Pillow 的 `Image.thumbnail()` 方法，保持宽高比，最大边 200px。缓存文件命名为 `{image_id}.png`。
...
> 由工具栏"**源文件链接刷新**"按钮触发，检查数据库中图片记录与磁盘文件的一致性。

```mermaid
flowchart TD
    A[用户点击源文件链接刷新按钮] --> B[遍历所有 images 记录]
    B --> C{文件是否存在?}
    C -- 是 --> D{文件哈希是否变化?}
...
> 由工具栏"**缩略图生成刷新**"按钮触发，**始终刷新全部**缩略图，不检查缩略图是否已存在，直接对每张图片重新生成。

```mermaid
flowchart TD
    A[用户点击缩略图生成刷新按钮] --> B[遍历所有 images 记录]
    B --> C[对每张图片重新生成缩略图<br/>覆盖缓存目录中对应 PNG 文件]
    C --> D{原图文件是否存在?}
    D -- 是 --> E[生成新缩略图]
    D -- 否 --> F[跳过该图片<br/>保留默认占位缩略图]
    E --> G[更新 UI]
    F --> G
```
...
- **性能**：源文件链接刷新与缩略图生成刷新均在后台线程执行，避免阻塞 UI。缩略图生成刷新因始终遍历全部图片，耗时较长，需显示进度。
...
| **源文件链接刷新按钮** | 触发文件索引有效性检查（文件存在性、哈希比对） |
| **缩略图生成刷新按钮** | 重新生成全部缩略图（始终刷新全部，不检查是否已存在） |
...
| 详细信息 | 表格行 | 除包含图片基本信息外，每一个标签类占据一列，列内容是该类的标签 | 支持点击列头排序 |
...
| **源文件链接刷新线程** | 文件存在性检查、哈希比对 | 通过 Signal 发送更新结果 |
| **缩略图生成刷新线程** | 遍历全部图片重新生成缩略图 | 通过 Signal 发送进度/结果 |
...
class ThumbnailRefreshWorker(QThread):
    """缩略图生成刷新线程（始终刷新全部）"""
    progress = pyqtSignal(int, int)          # 当前进度, 总数
    thumbnail_updated = pyqtSignal(int)      # image_id
    finished = pyqtSignal(int)               # 生成完成, 成功数量
```
...
| **文件被外部删除** | 源文件链接刷新后标记 `is_missing=1`，条目置灰 + ⚠ 图标 |
| **文件被外部移动** | 源文件链接刷新后标记丢失，提供"重新连接文件" |
| **文件被外部修改** | 源文件链接刷新后更新哈希、尺寸、缩略图 |
...
| 删除外部文件 → 源文件链接刷新 | 丢失标记正确显示 |
| 缩略图缓存清空 → 缩略图生成刷新 | 全部缩略图重新生成 |
| 重连丢失文件 | 路径更新，标签保留 |
| 备份 → 修改数据 → 恢复 | 数据完整恢复 |
...
| **P1** | 分类管理、标签管理、搜索、源文件链接刷新、丢失标记、视图切换、右栏 | M2-M3 |
| **P2** | 批量添加标签、缩略图生成刷新、详细信息视图、通用交互 | M3-M4 |
